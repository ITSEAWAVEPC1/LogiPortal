...
export function ReportTable({ table }: { table: ReportTableType }) {
  ...
  const [titleColumn, ...restColumns] = table.columns;
    ...
    <>
      ...
      <div className="flex flex-col gap-3 lg:hidden">
        {table.rows.map((row, i) => (
          <MobileRowCard
            key={i}
            title={formatReportCell(row[titleColumn.key], titleColumn) || titleColumn.header}
            rows={restColumns.map((c) => ({ label: c.header, value: formatReportCell(row[c.key], c) }))}
          />
        ))}
        {table.total ? (
          <MobileRowCard
            emphasized
            title="Total"
            rows={restColumns.map((c) => ({ label: c.header, value: formatReportCell(table.total?.[c.key], c) }))}
          />
        ) : null}
      </div>
    </>
