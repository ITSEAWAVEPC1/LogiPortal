...
- `src/middleware.ts`'s matcher excludes `/api/*` entirely — there is **no edge-level auth on API routes**. Every route handler must call `auth()` and check permissions itself (see any existing route under `src/app/api/` for the pattern).
