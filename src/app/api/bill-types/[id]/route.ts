...
const billTypeUpdateSchema = z.object({
  name: z.string().trim().min(1).optional(),
  code: z
    .string()
    .trim()
    .min(1)
    .transform((v) => v.toUpperCase().replace(/[^A-Z0-9]+/g, "_").replace(/^_+|_+$/g, ""))
    .optional(),
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = billTypeUpdateSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ error: "Validation failed", issues: parsed.error.issues }, { status: 400 });
  }

  const action = typeof parsed.data.isActive === "boolean" ? "delete" : "edit";
  if (!can(session.user.role, "billTypes", action)) {
  ...
  const billType = await prisma.billType.update({ where: { id }, data: parsed.data });
